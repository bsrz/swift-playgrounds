...
class ControlTests: XCTestCase {
    ...
    func testViewModel_whenInstantiated_createsInitialState() {
        let sut = Control.BasicViewModel(makeDate: makeDate, makeUUID: makeUUID)
    ...
    func testViewModel_whenSentResetIdAction_updatesStateWithNewId() {
        let sut = Control.BasicViewModel(makeDate: Date.init, makeUUID: makeUUID)
    ...
    func testViewModel_whenSentUpdateAction_updatesStateWithNewDate() {
        let sut = Control.BasicViewModel(makeDate: makeDate, makeUUID: UUID.init)
        let previous = sut.state
